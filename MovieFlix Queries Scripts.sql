...
SELECT s.staff_id, SUM(p.amount) AS total_revenue
FROM staff s
JOIN payment p ON s.staff_id = p.staff_id
GROUP BY s.staff_id

-- 6. Find the number of customers in each store. (Not in Question Slide)
...
CASE WHEN SUM(amount)>100 THEN 'High Spender'
	WHEN SUM(amount) BETWEEN 70 AND 100 THEN 'Middle Spender'
	ELSE 'Low Spender' 
...
FROM film f
JOIN rental r ON r.inventory_id = i.inventory_id
JOIN inventory i ON f.film_id = i.film_id  --the 2 joins were interchanged to confirm if the query above would still work--
GROUP BY f.title
ORDER BY num_of_rent DESC
LIMIT 10     -----Error output confirming that joins must be in an orderly manner-------
...
FROM payment
WHERE payment_id IS NOT NULL AND rental_id IS NULL

SELECT c.customer_id, c.first_name || '-' || c.last_name AS full_name 
FROM customer c  --TO SHOW HOW CONCATENATE WORKS IN SQL---

-- Which staff made the most rental
--and how much revenue have they handled

SELECT staff_id, SUM(amount) AS revenue, COUNT(rental_id) AS num_rent
FROM payment p
GROUP BY staff_id 
...
SELECT i.store_id, ROUND(AVG(f.rental_duration),3) AS "average rental duration" ---ROUND FUNCTION USED TO ROUND UP DECIMAL PLACES--
FROM film f
JOIN inventory i ON i.film_id = f.film_id
GROUP BY i.store_id
